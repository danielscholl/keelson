...
- **Bespoke SVG** for the signature hero illustration, "The Frame"
  ...
  beam is the harness, the labeled ribs are brass frame members bolted to the
  beam (a midship section inset shows one rib's true curve on the keelson), the
  CLI is the rudder (it steers from the shell), the browser SPA is a surface,
  and a dashed superstructure (wheelhouse, mast, and rigging) marks capability
