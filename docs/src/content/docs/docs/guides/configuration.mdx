...
| `KEELSON_MAX_CONCURRENT_RUNS` | `2` | Process-wide ceiling on workflow runs executing at once; further runs queue. Raise on a larger host. |
| `KEELSON_CROSS_RIB_GRANTS` | unset | `;`-separated `caller:target:tool` grant triples (the tool field takes a comma-separated list or `*`); default-deny when unset. See [Governance](../governance/#cross-rib-grants). |
