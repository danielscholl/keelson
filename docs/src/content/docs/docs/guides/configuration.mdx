...
server and is read-only and tokenless on loopback by default. The `mcp` block
tunes it; [Using keelson over MCP](../using-mcp/) is the task guide.
...
| `exposeStateChanging` | boolean | Also expose state-changing tools (`workflow_run`, `workflow_respond`, a rib's mutating tools). Default `false`, so only read-only tools cross. |
...
| `KEELSON_MCP_EXPOSE_STATE_CHANGING` | unset | Set to `1` to expose state-changing tools over MCP. Overrides `mcp.exposeStateChanging`. |
