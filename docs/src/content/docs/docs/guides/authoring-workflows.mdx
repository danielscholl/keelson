...
[Run and author a workflow](../../tutorials/run-a-workflow/).
...
work is not repeated (a node marked `always_run` re-runs anyway). You can also
resume a terminated run by asking the agent (the `workflow_resume` chat/MCP tool)
...
The variable is capped at 16 KiB and head+tail truncated past that, with a
...
`keelson workflow validate` warns when a shell body parses the capped variable
directly. See [the env channel](../../reference/workflow-nodes/#data-flow) for
the full table.
...
- [Workflows](../../concepts/workflows/): the execution model and the run
  lifecycle.
- [Run and author a workflow](../../tutorials/run-a-workflow/): a narrated first
