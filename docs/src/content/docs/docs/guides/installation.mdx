...
names the agents it is about to disconnect. If one of them refuses the removal —
its own CLI is gone, or errors — the command reports that agent as failed, exits
non-zero, and keeps its entry in the receipt so a reinstall can still reverse it.
