...
`POST /api/mcp` exposes only read-only tools by default and can be gated behind a
bearer token. Treat the surface as stable for reading and local automation, not
...
| `POST` | `/api/mcp` | The MCP gateway: the same tool registry over the Model Context Protocol, for external agents. Read-only and tokenless by default; `GET` returns 405 (no SSE stream). See [Using keelson over MCP](../../guides/using-mcp/). |
...
| `WS` | `/api/snapshots/:key/ws` | Subscribe to a key: the latest frame on connect, then a `snapshot_update` on every recompose. |
...
Three resources stream over WebSocket, all under the same base URL. Each sends
the current state on connect, then incremental frames.
...
| `/api/snapshots/:key/ws` | The cached frame on connect, then a `snapshot_update` per recompose. |
