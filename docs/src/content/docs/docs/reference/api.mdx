...
Two endpoints are special: `POST /api/server/shutdown` is gated by the token in
`server.json` and is operator-internal, and `POST /api/ribs/:id/action` is
loopback-trusted today (the capability-token envelope is a later milestone).
Treat the surface as stable for reading and local automation, not as a versioned
public contract.
</Aside>
...
| `GET` | `/api/config` | The resolved configuration the server booted with. |
...
| `GET` / `PATCH` / `DELETE` | `/api/projects/:id` | Read, update, or remove a project. |
