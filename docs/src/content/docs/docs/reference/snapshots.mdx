...
The header's `people` is a roster peek: an identity-toned dot per person, names
...
same name-required rule the [identity tones](#tones) state. `defaultCollapsed` is
...
Each section is discriminated on `kind`. Eleven are leaf sections; `columns` is a
...
| `bars` | `inline?`, `items: [{ label, value, total, tone?, trailing?, href? }]` | Meters and offender lists. A safe `http(s)://` `href` makes the whole bar row a link; other schemes fall back to plain text. |
| `table` | `columns`, `rows`, `caption?` | The same table shape as the view. |
| `cards` | `boxed?`, `grid?`, `items: [{ title, titleTone?, mono?, stacked?, dot?, pill?, href?, bar?, fields?, actions?, footnote?, ghost?, reason? }]` | Rich item cards. A card may carry `actions: ActionItem[]` for inline per-card buttons, distinct from a standalone `actions` section. `stacked` renders the card's fields as a column (one per line) instead of the inline `·`-joined meta row — for line-oriented readouts. `grid` lays the cards out side by side as an auto-fit grid instead of the stacked full-width column — for fixed-capacity rosters where the row is the bench; the host owns the responsive column count, and an open fields-form stays inside its card's column. A `ghost` card renders as an open-seat placeholder (dashed border, centered body) — the empty-seat affordance in a grid bench, e.g. an authoring launchpad or an unfilled casting slot; while a ghost card's form is open it sheds the dashes for a solid brand edge. |
...
ActionItem = {
  ...
  glyph?, tone?,
  destructive?,         // marks the action dangerous: a simple confirm
  inline?,              // surface a destructive card action as a visible button, not the overflow menu
  confirm?: Confirm,    // typed/irreversible confirm-dialog metadata, distinct from the boolean destructive
...
An `ActionField` is `{ name, label, placeholder?, required?, multiline?,
options?, modelPicker?, defaultValue? }`. When a button declares `fields`,
clicking it opens a small form, and the collected `{ name: value }` map is merged
into the dispatched payload. Field names must be unique within an action.
`options` (an array of `{ value, label }`) renders a select instead of a text
input: the dispatched value is the chosen option's `value`, a non-required select
offers `placeholder` as an empty "none" option, and the option values must be
unique. It is mutually exclusive with `multiline`. `defaultValue` pre-fills the
control; for a select it must name an option value or `""`.
...
`confirm` is typed confirmation-dialog metadata,
`{ irreversible?, subject?, title?, body?, label?, confirmLabel?, cancelLabel? }`,
distinct from the boolean `destructive`. `destructive` still marks the action
dangerous; `confirm` only controls whether the confirm dialog is simple or typed,
and an `irreversible` confirm requires a `subject` the operator types before
confirm enables.
