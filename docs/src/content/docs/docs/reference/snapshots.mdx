...
| `seats` | `items: [{ tone?, filled?, label? }]` | A fixed-capacity identity row — seats an actor roster fills. A dashed empty seat becomes solid when `filled`; a `label` (the actor's name) becomes the seat's tooltip and accessible name, and an unlabelled seat is decorative (`aria-hidden`), so identity colour never renders as an identity claim without a name. |
