...
`options`, and `providerField` must differ from the field's own `name`. When the
picker is the action's **only** field, the host skips the intermediate form: the
action button opens the catalog directly and picking dispatches immediately
(dismissing the popover is the cancel).
