...
ActionItem = {
  ...
  selected?,            // a toggle's ON state (brand ring + aria-pressed). Absent ≠ false: absent is a plain verb action with no pressed semantics, false is an off toggle. Only on a click-dispatching action: no fields, not destructive, not in a tabs strip (which owns its own active state) and not a headAction — each of those renders through a control with no pressed state
