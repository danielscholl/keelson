...
ActionItem = {
  ...
  defaultOpen?,         // in a tabs strip, open the first enabled, field-bearing item marked true on first render — the operator's toggles win after (inert elsewhere and without fields)
...
An `ActionField` is `{ name, label, placeholder?, required?, multiline?, half?,
segmented?, options?, modelPicker?, defaultValue? }`. When a button declares
...
action. `options` (an array of `{ value, label }`) renders a select instead of a
...
select's empty option — and is rejected without `options`.
