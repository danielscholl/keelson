...
options?, defaultValue? }`. When a button declares `fields`, clicking it opens a
small form, and the collected `{ name: value }` map is merged into the dispatched
payload. Field names must be unique within an action. `options` (an array of
`{ value, label }`) renders a select instead of a text input: the dispatched
value is the chosen option's `value`, a non-required select offers `placeholder`
as an empty "none" option, and the option values must be unique. It is mutually
exclusive with `multiline`. `defaultValue` pre-fills the control; for a select it
must name an option value or `""`.
