...
| `cards` | `boxed?`, `items: [{ title, titleTone?, mono?, dot?, pill?, href?, bar?, fields?, actions?, footnote?, reason? }]` | Rich item cards. A card may carry `actions: ActionItem[]` for inline per-card buttons, distinct from a standalone `actions` section. |
...
| `actions` | `wrap?`, `items: ActionItem[]` | Buttons. `wrap` lays them out as an inline wrapping row of compact chips (a selection strip) instead of a full-width stacked column; an action whose form is open breaks to its own full-width line. See below. |
...
A card `field` is `{ label?, value, tone?, href?, copyable?, copyAction? }`. A
card may also carry a `reason: { label?, text }` annotation line.
