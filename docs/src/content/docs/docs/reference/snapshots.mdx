...
ActionItem = {
  ...
  selected?,            // a toggle's ON state (brand ring + aria-pressed). Absent ≠ false: absent is a plain verb action with no pressed semantics, false is an off toggle. Carries no fields, and not allowed in a tabs strip (which owns its own active state)
