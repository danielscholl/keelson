...
it; `keelson provider remove <id>` reverses both. An enabled provider whose SDK
...
| Claude | `claude` | `provider add` | yes | yes | yes | keelson keychain (subscription or API key) |
| Codex | `codex` | `provider add` | yes | yes | no | self-managed |
| Pi (community) | `pi` | `provider add` | no | yes | yes | self-managed |
