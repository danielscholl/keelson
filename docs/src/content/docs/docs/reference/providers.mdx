...
`capabilities.models` starts as a baseline. Claude and Codex publish known ids;
Copilot discovers the ids available to your account. `GET
/api/providers/:id/models` returns the live list the picker prefers.
...
[per-provider `modelClasses` settings](../../guides/configuration/#model-classes).
...
`deep` from concrete models' cost tiers, excluding `auto` from the ranking; entries
without cost metadata use catalog order. Its published models then include `auto`
and the discovered concrete ids. A class-based Copilot turn waits for that bounded
fetch if it is still in progress. An explicit model or an unclassed turn never
waits for it, and the unclassed default remains `auto`.
...
Pi is multi-vendor and resolves its catalog from the vendors your credentials
authenticate, so it also has no fixed list to publish.
