...
Model selection starts with the node's `model`, then the workflow's `model`. A
`model_by_provider` entry for the effective provider takes precedence, and a
[`model_by`](#model_by) case that matched replaces both. A model
