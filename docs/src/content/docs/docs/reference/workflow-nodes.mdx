...
| `always_run` | all | `true` re-executes the node on a resumed run even if it previously succeeded (default `false`). Its transitive descendants also re-run, including prompt nodes that make provider calls and incur their normal token cost. Use it for a gate or validation that must re-check rather than replay a prior pass: see [resume and side effects](#resume-and-side-effects). |
...
`keelson workflow resume` seeds reusable succeeded nodes as complete. Failed,
...
When a node re-executes, all of its transitive descendants re-execute too. This
keeps every derived output consistent with the new ancestor output. It also
means a descendant prompt node makes another provider call and incurs its normal
token cost. Independent branches remain seeded.
...
A fully seeded [converge](#converge) subgraph remains seeded. If invalidation
reaches its gate, the full converge ancestor subgraph re-executes from round 1,
and affected descendants outside the subgraph re-execute with it.
