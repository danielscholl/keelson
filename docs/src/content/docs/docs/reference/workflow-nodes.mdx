...
fields from [common fields](#common-node-fields). A `loop` also takes node-level
...
| `model_by` | `prompt`, `command` | Pick the model and/or effort from run data: see [model_by](#model_by). |
...
| `output_schema` | all | A JSON-Schema subset the output must satisfy, or the node fails. |
...
For `prompt` and `command`, a matched [`model_by`](#model_by) case is applied
