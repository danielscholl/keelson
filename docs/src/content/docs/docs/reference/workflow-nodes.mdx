...
For prompt nodes, a new run started from the CLI, the SPA, or the API checks
pinned literal model ids and effort against the effective provider's live
catalog before executing its first node. A resumed run keeps the result it
already passed rather than re-checking, and a run started by a rib, such as a
region's cadence refresh, skips the check entirely. Run the same check without
executing with
...
The referenced node must be an ancestor prompt. Comparison uses the effective
provider and model recorded after fallback and provider-reported model changes.
When both models map to the same known vendor, the run emits a warning. An
unknown model vendor is not treated as proof that the turns are diverse.
...
| `KEELSON_RUN_ID` | The current workflow run id. |
