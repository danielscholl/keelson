...
| `KEELSON_NODE_<id>_OUTPUT_FILE` | Path to a file holding that output in full, always set. Best-effort: omitted only when the run has no artifacts directory or the write fails. |
...
structured output from the file, not the variable** — unconditionally, since the
path is always set:
