...
| `always_run` | all | `true` re-executes the node on a resumed run even if it previously succeeded (default `false`). Use it for a gate or validation that should re-check rather than replay a stale pass, and for any node whose real product is a file: see [resume and side effects](#resume-and-side-effects). |
...
`model_by_provider` entry for the effective provider takes precedence. A model
...
| `KEELSON_NODE_<id>_OUTPUT_FILE` | Path to the untruncated output, set only when that output was capped. Best-effort: omitted when the run has no artifacts directory or the write fails. |
...
environment entry. A truncated node output is also written in full to
`<artifacts dir>/node-outputs/<id>.txt` and that path published as
`KEELSON_NODE_<id>_OUTPUT_FILE`, so a node that needs the whole text reads the
file instead of the variable.
...
A node whose real product is a **side effect** does not. A collector that writes
into `$KEELSON_ARTIFACTS_DIR` and succeeds during an attempt that later fails is
skipped on resume, so the file it owns still holds what the failed attempt left
there, and downstream nodes read that. Mark such a node `always_run: true`.
