...
Workflows live in two scopes, and the catalog merges them:
...
A project workflow **shadows** a same-named global one within its project. Ribs
also contribute workflows into the catalog at activation, optionally
[bound to a snapshot key](../snapshots/#bound-workflows).
