...
For prompt nodes, a new run checks pinned literal model ids and effort against
the effective provider's live catalog before executing its first node. Run the
same check without executing with
