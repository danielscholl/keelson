...
and which fails loudly on its own. That is the difference from the old behavior,
where the path appeared only once an output happened to outgrow the cap, so the
same workflow worked for a dozen runs and then did not.
