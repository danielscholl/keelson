...
| `KEELSON_NODE_<id>_STATE` | The upstream node's recorded execution state. |
| `KEELSON_NODE_<id>_ERROR` | The upstream node's error text when its state is `failed`; unset otherwise. |
| `KEELSON_NODE_<id>_PROVIDER` | The provider that handled the upstream node; unset when no provider was recorded. |
| `KEELSON_NODE_<id>_MODEL` | The model that handled the upstream node; unset when no model was recorded. |
