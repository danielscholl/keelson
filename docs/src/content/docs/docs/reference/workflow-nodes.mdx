...
provider's live catalog before the first node executes. A resumed run keeps the
result it already passed rather than re-checking, and a run started by a rib,
such as a region's cadence refresh, skips the check entirely. Run the same check without executing with
