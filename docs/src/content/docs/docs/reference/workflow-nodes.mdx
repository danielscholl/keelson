...
A new run checks pinned literal model ids and effort against the effective
provider's live catalog before executing its first node. Run the same check
without executing with `keelson workflow validate <name> --live`. An unavailable
catalog is reported as `not checked` and does not block the run. A model that
reports no effort list is not judged on effort.
