...
[Using keelson over MCP](../../guides/using-mcp/#connect-an-agent-with-one-command).
...
[Providers](../providers/#the-provider-matrix).
...
server is running, and reports each installed rib's package version. Most other
rib commands talk to the running server and report `3` when it is down.
...
| Command | Does |
| --- | --- |
| `keelson doctor` | Probe the toolchain, server, database, auth, workflows, and ribs, including whether the [cross-rib grants](../../guides/configuration/#cross-rib-grants) the running server holds still resolve. `--strict` exits non-zero on warnings too. |
...
| `keelson uninstall` | Remove the program files, the launcher, and the keychain entries keelson wrote. `--purge` also deletes the home; `--yes` skips the prompt; `--keep-credentials` leaves the keychain alone; `--force` proceeds when the server could not be stopped. |
...
`package.json`, `bun.lock`, `.npmrc` — are what `install.sh` provisioned, and
they always go. Your data — `keelson.db`, `workflows/`, `commands/`,
`config.json`, and each rib's data directory — stays unless you pass `--purge`.
Running it twice is the two-step form: a plain run removes the program files,
then `--purge` deletes what is left. A home with a `package.json` that is not
keelson's own is refused outright, so pointing the command at a source checkout
cannot cost that project its `node_modules`.

<Aside type="caution" title="Rib credentials outlive the command">
The OS keychain resolves entries by exact name and cannot be enumerated, so
`keelson uninstall` revokes only the accounts it knows it wrote: provider keys
and configured gateways. A rib that stores secrets under its own service ids is
outside what the command can find. It names the installed ribs and the
`rib_<id>_*` pattern on the way out; clear those by hand.
</Aside>
