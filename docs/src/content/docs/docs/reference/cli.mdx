...
| Command | Does |
| --- | --- |
| `keelson connect <agent...>` | Wire an external agent (`claude`, `copilot`, `codex`, or `all`) to the MCP endpoint and drop a portable skill at `.agents/skills/keelson/SKILL.md`. `--no-skill` wires the connection only; `--url <url>` writes a non-default endpoint; `--undo` reverses; `--list` shows current connections. |
