...
| `keelson worktree prune` | Remove worktrees and `keelson/…` branches left by finished isolated runs. `--dry-run` lists candidates; `--force` also removes live runs' worktrees and dirty ones. |
