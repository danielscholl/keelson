...
An agent reads its MCP config only at startup, so a connection made mid-session is
invisible until it re-reads. Restart the agent, or open a new session, so it picks
up the endpoint. Verify what is wired with:

```sh
keelson connect --list
```

If `keelson connect` reported a problem, it exits `1` when any target fails and
...
See [Using keelson over MCP](../../guides/using-mcp/) for the wiring each agent uses.
...
The update re-pinned to the new release, but `bun install` could not resolve the
...
The toolchain category's `npm registry` check reports the effective registry.
