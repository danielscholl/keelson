...
| Plan | `investigate` | prompt | **`when: issue_type == 'bug'`**, **`model: gpt-6-astra`**, **`effort: high`**: root-cause analysis into `plan.md`. |
| | `plan` | prompt | **`when: issue_type != 'bug'`**, **`model: gpt-6-astra`**, **`effort: high`**: an implementation plan into the *same* `plan.md`. |
...
`plan`) are pinned to `gpt-6-astra`; the judge nodes (`coverage-check`,
`triage`, `triage-ci`) to `claude-opus-4.8`; the edit nodes (`implement`,
`fix-validation`, `apply-fixes`, `fix-ci`) to `gpt-5.6-sol`. The review panel is
mixed: `review-correctness` runs `gpt-6-astra`, while `review-conventions`,
`review-coverage`, and `re-review` run `gpt-5.6-terra`. The implementer never
...
- **Tune the review loop.** Phase 7's panel (`capture-diff`,
  ...
  the last step. The pins come in four roles (`gpt-6-astra` for planning
  and the correctness lens, `gpt-5.6-terra` for the other review nodes,
  ...
  if you can. All four ids are account-dependent: swap or drop any to `auto` if it
