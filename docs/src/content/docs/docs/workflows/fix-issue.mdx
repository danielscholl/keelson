...
| Draft PR | `create-pr` | prompt | Pushes the branch, opens a draft PR with `Fixes #N`, and captures the PR number and URL. It follows the repository's PR template exactly when one exists. Its test plan records commands actually executed and their results, with `pending CI` reserved for checks that only CI can confirm. |
