...
| | `extract-brief` | bash | Extracts bullets under acceptance criteria, requirements, definition of done, success criteria, or expected-behavior headings into `$ARTIFACTS_DIR/brief.json`. Heading suffixes such as `(must all hold)` are accepted. |
