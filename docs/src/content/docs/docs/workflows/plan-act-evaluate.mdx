...
It needs `gh` or `glab` authenticated against a repo with issues enabled (the
...
- **External signal as the gate**: `forge pr checks` as the authoritative check, over the agent's own confidence.
