...
| `code-review` | prompt | **`model: gpt-5.6-sol`**, **`effort: xhigh`**: the always-on finder is the recall ceiling, so it gets the strongest coding model, from a different family than the triage judge. Hunts highest-cost failures first (trust boundaries, irreversible state, idempotency gaps), labels unverified external-API claims INFERRED, and runs unless the PR is non-code. |
...
| `triage` | prompt | **`model: claude-opus-4.8`**, **`effort: xhigh`**, `allowed_tools: [Read, Glob, Grep]`, the `one_success` join: de-dupes the lanes, then adversarially refutes each candidate against `diff.patch` and the source. Survivors are CONFIRMED (confidence 85-100) or PLAUSIBLE (capped at 79, below the ≥80 keep line), so only CONFIRMED CRITICAL/HIGH/MEDIUM findings post. Emits a `verdict` (READY TO MERGE / NEEDS NITS / NEEDS FIXES) with typed `findings`, plus a `summary` written in the posting engineer's first-person voice — it becomes the review body verbatim, with no branding or verdict banner. |
| `build-review` | bash | Offline: anchors each finding to a line on the diff's new side and builds the review payload. A `suggestion` block is **gated**: an empty fix, or one byte-identical to the anchored line, posts as prose only. No network. |
...
`diff.patch` and the source it points at. A candidate dies when the code already
handles the case, the claim misreads the diff, the issue predates the PR, or a
linter or CI gate already catches it. A survivor is **CONFIRMED** when the judge
...
- **Repoint the output.** Swap the `post-review` step's `forge pr review-batch`
  call for a different sink (a single summary comment, a file) without touching
  the classify, lane, or triage logic.
