...
| `triage` | prompt | **`model: claude-opus-4.8`**, `allowed_tools: [Read, Glob, Grep]`, the `one_success` join: de-dupes the lanes, then adversarially refutes each candidate against `diff.patch` and the source. Survivors are CONFIRMED (confidence 85-100) or PLAUSIBLE (capped at 79, below the ≥80 keep line), so only CONFIRMED CRITICAL/HIGH/MEDIUM findings post. Emits a `verdict` (READY TO MERGE / NEEDS NITS / NEEDS FIXES) with typed `findings`. |
