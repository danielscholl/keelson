...
file or literal guidance. `tier` is `deep` or `std`; `verifier` is `claude` or
...
| --- | --- | --- |
| `intake` | bash | Validates the question, output path, tier, and verifier; bundles context and access guidance; rotates the default verifier from the run id. |
