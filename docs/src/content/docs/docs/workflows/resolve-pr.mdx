...
| `apply-review-fix` | prompt | **`model: gpt-6-sol`**, **`effort: high`**. Tries to refute each CRITICAL or HIGH finding at confidence 80 or above, fixes the ones that hold, commits without pushing, and logs the outcome per finding in `pre-push-review.md`. |
