...
export const LATEST_MIGRATION_VERSION = 9;
...
export async function runDbCheck(deps: DbDeps = {}): Promise<CategoryResult> {
  ...
  if (!exists(path)) {
    const check: CheckResult = {
      ...
      hint: "run `keelson serve` once to create and migrate the SQLite database",
    };
  ...
  try {
    ...
    const check: CheckResult = {
      ...
      hint: "the SQLite file may be corrupt or locked; stop any running `keelson serve` and try again",
    };
    return { category: "db", checks: [check] };
  }
  ...
  if (current === null) {
    const check: CheckResult = {
      ...
      hint: "run `keelson serve` once to apply migrations",
    };
    return { category: "db", checks: [check] };
  }
  ...
  if (current < latest) {
    const check: CheckResult = {
      ...
      hint: "run `keelson serve` to apply pending migrations",
    };
    return { category: "db", checks: [check] };
  }
