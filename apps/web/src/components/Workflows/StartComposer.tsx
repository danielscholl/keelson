...
export interface StartComposerProps {
  // Active project id sourced from the header chip. Null gates the Start
  // button — the composer doesn't own project selection anymore.
...
export function StartComposer({ projectId, onStart, starting }: StartComposerProps) {
  const [text, setText] = useState("");
  const canStart = !starting && projectId !== null;

  const submit = async () => {
    if (!canStart || !projectId) return;
    await onStart({ args: text, projectId });
  };
  ...
  return (
    <div className="start-composer">
      <textarea
        ...
        onKeyDown={onKeyDown}
        placeholder="Instructions — Enter to start, Shift+Enter for newline"
        rows={1}
        disabled={starting}
