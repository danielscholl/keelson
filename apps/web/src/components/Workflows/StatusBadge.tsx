...
// Pill renderer shared by run header + recent-runs table + (W4.6) HITL
// callouts. Status → class mapping mirrors mockups/workflows-page.html's
// `.status-pill.{state}` lookups so future Tailwind/CSS-only consumers
// can pick up the class set directly.
...
// CSS class names align with mockup. `succeeded` maps to `.completed` so
// designers see one canonical visual state across both pill and dot.
...
// the row (catalog list items, sidebar entries in W4.5).
