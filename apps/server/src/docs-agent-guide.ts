...
4. **Inspect.** Use \`workflow_status\` to check a run's progress or to read a completed run's node outputs. For a poll loop, pass \`brief: true\` to the MCP tool. Its compact result includes node id/status, live \`current\`, the awaiting node, and \`pauseId\`, so you can resume an approval without fetching every node's output. The CLI \`workflow status --brief\` is for compact operator inspection: it lists nodes and names only the awaiting node as current, without \`pauseId\`, so do not use it for an approval-resuming agent loop.
