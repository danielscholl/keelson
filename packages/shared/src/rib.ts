...
// surface, a cadence refresh. `inputs` are the run's inputs, so a rib can
// reconstruct dispatch context for runs it didn't launch; `error` carries the
// run-level failure message on a failed run. See Rib.onRunEvent.
