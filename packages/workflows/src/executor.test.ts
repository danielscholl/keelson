...
describe("runWorkflow — status-report (2 layers, sequential)", () => {
	...
	test("emits node_started in topological order [collect, summarize]", async () => {
		...
		const started = events
			.filter((e): e is Extract<RunStreamEvent, { type: "node_started" }> => e.type === "node_started")
			.map((e) => e.nodeId);
...
`);
	...
	test("\\$ARTIFACTS_DIR escape preserves the literal token in the body", async () => {
		...
		expect(calls[0].resolvedBody).toBe(
			"doc: $ARTIFACTS_DIR is the run scratch dir",
		);
	});
...
describe("runWorkflow — DAG validation", () => {
	...
	test("duplicate node id throws ExecutorValidationError", async () => {
		const workflow: WorkflowDefinition = {
			...
			nodes: [
				{ id: "same", bash: "echo 1" } as DagNode,
				{ id: "same", bash: "echo 2" } as DagNode,
			],
		};
		await expect(
			runWorkflow({
...
`);
		...
		const reviewDone = events.findIndex(
			(e) => e.type === "node_done" && e.nodeId === "review",
		);
