...
describe("formatSubprocessFailure", () => {
  test("includes exit code suffix when provided", () => {
    expect(
      formatSubprocessFailure("Script node 'x'", { cmd: "bun", exitCode: 1, stderrTail: "oops" }),
    ).toBe("Script node 'x' failed [exit 1]: oops");
  });
  ...
  test("truncates very long stderr tails", () => {
    ...
    const msg = formatSubprocessFailure("Bash node 'big'", {
      cmd: "bash",
      exitCode: 7,
      stderrTail: tail,
    });
