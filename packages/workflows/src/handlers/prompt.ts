...
export function makePromptHandler(opts: MakePromptHandlerOptions): NodeHandler {
	...
	return {
		...
		async handle(node, ctx): Promise<NodeResult> {
			...
			const model =
				typeof nodeModel === "string" && nodeModel.length > 0
					? nodeModel
					: undefined;
			...
			const nodeDeniedBare =
				nodeDenied === undefined ? undefined : nodeDenied.map(stripMcpPrefix);
			...
			if (nodeAllowedBare !== undefined) {
				...
				filteredTools = allTools.filter(
					(t) => allowSet.has(t.name) && !denySet.has(t.name),
				);
			} else {
				const effectiveDeny =
					nodeDeniedBare === undefined
						? denySet
						: new Set([...denySet, ...nodeDeniedBare]);
			...
			const consume = async (): Promise<void> => {
				try {
					const stream = opts.getProvider().sendQuery(
						ctx.resolvedBody,
						ctx.cwd,
						undefined,
						{
							...
							...(nodeAllowed !== undefined
								? { allowedTools: nodeAllowed }
								: {}),
							...(nodeDenied !== undefined
								? { disallowedTools: nodeDenied }
								: {}),
...
function readHooksField(
	node: unknown,
): PromptHandlerSendOptions["hooks"] | undefined {
